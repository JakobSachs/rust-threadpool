...
use rust_threadpool::pools::v6_condvar_completion;
use std::sync::Arc;
...
fn main() {
    ...
    let pool_size = std::env::var("POOL_SIZE")
        ...
        .parse()
        .unwrap();
    let pool = v6_condvar_completion::Pool::new(pool_size);
    ...
    println!(
        "[V6-condvar-completion T={}] Processed {} numbers in {:.3}s ({:.2} k-numbers/sec)",
        pool_size,
        TASK_COUNT,
        elapsed.as_secs_f64(),
        per_second
